...
export async function GET(req: NextRequest) {
  try {
    ...
    const presets = getAdminPresets(includeInactive);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "REORDER") {
      ...
      const presets = reorderAdminPresets(orderedIds);
      return NextResponse.json({ success: true, presets });
    }

    if (action === "RESET") {
      const presets = resetAdminPresets();
      return NextResponse.json({ success: true, presets });
    }

    if (action === "CLONE") {
      const { sourcePresetId, newId, newTitleKo, newTitleEn } = body;
      const presets = getAdminPresets(true);
      ...
      const created = createAdminPreset(clonedPreset);
    ...
    const created = createAdminPreset(preset);
    return NextResponse.json({ success: true, preset: created });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const updated = updateAdminPreset(id, updates);
