"use client";

import React from "react";
import { SupportedCity, TripDraft, CITY_KOREAN_NAMES, CITY_ENGLISH_NAMES } from "../lib/trip-domain";
import { IntercityTransportMode, IntercityFareInfo, getIntercityFareOptions } from "../lib/transport/intercity-fares";
import { formatKrw } from "../features/budget/presentation/formatters";
import type { Dictionary } from "../lib/i18n/dictionaries/ko";
import type { Locale } from "../lib/i18n/locales";

interface TransportPlannerPanelProps {
  draft: TripDraft;
  intercityOverrides: Record<string, IntercityTransportMode>;
  onSelectIntercityOverride: (routeKey: string, mode: IntercityTransportMode) => void;
  locale: Locale;
  dict: Dictionary;
}

export default function TransportPlannerPanel({
  draft,
  intercityOverrides,
  onSelectIntercityOverride,
  locale,
  dict,
}: TransportPlannerPanelProps) {
  const selectedCities = draft.selectedCities || [];
  const adultCount = draft.adultCount || 1;
  const isMultiCity = selectedCities.length >= 2;

  const getCityName = (city: SupportedCity) => {
    return locale === "ko"
      ? CITY_KOREAN_NAMES[city] || city
      : CITY_ENGLISH_NAMES[city] || city;
  };

  const getModeIcon = (mode: IntercityTransportMode) => {
    switch (mode) {
      case "KTX":
      case "SRT":
        return "🚆";
      case "EXPRESS_BUS":
        return "🚌";
      case "FLIGHT":
        return "✈️";
      case "FERRY":
        return "🚢";
      default:
        return "🚘";
    }
  };

  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white shadow-md border border-slate-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xl">🚆</span>
              <h3 className="text-lg font-extrabold tracking-tight">
                {locale === "ko" ? "교통수단 맞춤 설계 (Transport Planner)" : "Custom Transport Planner"}
              </h3>
              <span className="px-2 py-0.5 rounded-md bg-[#e25c5c] text-white text-[10px] font-black tracking-wider uppercase">
                {locale === "ko" ? "실시간 연동" : "Live Sync"}
              </span>
            </div>
            <p className="text-xs text-slate-300 font-medium">
              {locale === "ko"
                ? "구간별 최적의 이동 수단(KTX, 고속버스, 항공 등)을 비교하고 선택하세요. 선택 결과가 전체 예산에 실시간 반영됩니다."
                : "Compare and select the best transit options (KTX, Express Bus, Flight) for each segment."}
            </p>
          </div>
          <div className="shrink-0 px-3 py-1.5 rounded-xl bg-white/10 backdrop-blur-xs border border-white/15 text-xs text-center font-bold">
            {locale === "ko" ? `성인 ${adultCount}명 기준` : `For ${adultCount} travelers`}
          </div>
        </div>
      </div>

      {/* Part 1: 도시 간 이동 (Intercity Transit) */}
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-base">🛣️</span>
            <h4 className="text-sm font-extrabold text-slate-900">
              {locale === "ko" ? "도시 간 이동 구간 설정" : "Intercity Transit Segments"}
            </h4>
            <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[11px] font-bold">
              {isMultiCity ? `${selectedCities.length - 1}${locale === "ko" ? "개 구간" : " segments"}` : locale === "ko" ? "단일 도시" : "Single City"}
            </span>
          </div>
        </div>

        {!isMultiCity ? (
          <div className="p-6 rounded-2xl bg-slate-50 border border-dashed border-slate-300 text-center space-y-2">
            <span className="text-3xl">📍</span>
            <h5 className="text-sm font-bold text-slate-800">
              {locale === "ko" ? "단일 도시 여행입니다" : "Single City Trip"}
            </h5>
            <p className="text-xs text-slate-500">
              {locale === "ko"
                ? `${getCityName(selectedCities[0])} 단일 탐방 일정으로 도시 간 이동 교통비가 발생하지 않습니다.`
                : `No intercity transportation required for single city trip to ${getCityName(selectedCities[0])}.`}
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
            {Array.from({ length: selectedCities.length - 1 }).map((_, idx) => {
              const fromCity = selectedCities[idx];
              const toCity = selectedCities[idx + 1];
              const routeKey = `${fromCity}-${toCity}`;
              const options = getIntercityFareOptions(fromCity, toCity);

              const currentOverrideMode = intercityOverrides[routeKey] || intercityOverrides[`${toCity}-${fromCity}`];
              const activeOption = (currentOverrideMode && options.find((o) => o.mode === currentOverrideMode))
                || options.find((o) => o.isDefault)
                || options[0];

              const totalSegmentKrw = activeOption.oneWayPriceKrw * adultCount;

              return (
                <div
                  key={routeKey}
                  className="p-4.5 rounded-2xl bg-white border border-slate-200/90 shadow-2xs space-y-3.5 hover:border-slate-300 transition-all"
                >
                  {/* Route Header */}
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-2 font-black text-slate-900 text-sm">
                      <span className="px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-200 text-slate-800 font-extrabold text-xs">
                        {getCityName(fromCity)}
                      </span>
                      <span className="text-slate-400 font-normal">──►</span>
                      <span className="px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-200 text-slate-800 font-extrabold text-xs">
                        {getCityName(toCity)}
                      </span>
                    </div>

                    <div className="flex items-center gap-2 text-xs">
                      <span className="text-slate-500 font-medium">
                        {locale === "ko" ? "소계:" : "Subtotal:"}
                      </span>
                      <strong className="text-[#e25c5c] font-black text-sm">
                        {formatKrw(totalSegmentKrw)}
                      </strong>
                      <span className="text-[11px] text-slate-400">
                        ({formatKrw(activeOption.oneWayPriceKrw)} × {adultCount}{locale === "ko" ? "명" : ""})
                      </span>
                    </div>
                  </div>

                  {/* Mode Option Chips */}
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-500 block">
                      {locale === "ko" ? "이동 수단 선택" : "Select Transportation Mode"}
                    </label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      {options.map((opt) => {
                        const isSelected = activeOption.mode === opt.mode;
                        const badgeText = locale === "ko" ? opt.badgeTextKo : opt.badgeTextEn;

                        return (
                          <button
                            key={opt.mode}
                            type="button"
                            onClick={() => onSelectIntercityOverride(routeKey, opt.mode)}
                            className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between gap-1.5 ${
                              isSelected
                                ? "bg-slate-900 text-white border-slate-900 shadow-sm ring-2 ring-slate-900/20"
                                : "bg-slate-50 text-slate-700 border-slate-200/80 hover:bg-slate-100 hover:border-slate-300"
                            }`}
                          >
                            <div className="flex items-center justify-between gap-1">
                              <div className="flex items-center gap-1.5 font-extrabold text-xs">
                                <span>{getModeIcon(opt.mode)}</span>
                                <span>{locale === "ko" ? opt.nameKo : opt.nameEn}</span>
                              </div>
                              {badgeText && (
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[9px] font-black tracking-tight ${
                                    isSelected
                                      ? "bg-[#e25c5c] text-white"
                                      : "bg-slate-200 text-slate-600"
                                  }`}
                                >
                                  {badgeText}
                                </span>
                              )}
                            </div>

                            <div className="flex items-center justify-between text-[11px] font-medium opacity-90">
                              <span className={isSelected ? "text-slate-300" : "text-slate-500"}>
                                ⏱️ {locale === "ko" ? opt.durationTextKo : opt.durationTextEn}
                              </span>
                              <strong className={isSelected ? "text-amber-300 font-black" : "text-slate-900 font-extrabold"}>
                                {formatKrw(opt.oneWayPriceKrw)}
                              </strong>
                            </div>
                          </button>
                        );
                      })}
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </div>

      {/* Part 2: 도시 내 대중교통 정보 (City Local Transport) */}
      <div className="space-y-3 pt-3">
        <div className="flex items-center gap-2 border-b border-slate-200/80 pb-2.5">
          <span className="text-base">🚌</span>
          <h4 className="text-sm font-extrabold text-slate-900">
            {locale === "ko" ? "도시 내 대중교통 알뜰 패스 정보" : "City Transit Info"}
          </h4>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {selectedCities.map((city) => (
            <div key={city} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="font-extrabold text-xs text-slate-800">
                  📍 {getCityName(city)} {locale === "ko" ? "시내 교통" : "Local Transit"}
                </span>
                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200/60">
                  T-Money {locale === "ko" ? "연동" : "Compatible"}
                </span>
              </div>
              <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                {locale === "ko"
                  ? `${getCityName(city)} 시내 이동은 지하철 및 시내버스 기본 알뜰 패스로 자유롭게 탐방 가능합니다.`
                  : `Subway and city bus passes are fully available for convenient traveling in ${getCityName(city)}.`}
              </p>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}
