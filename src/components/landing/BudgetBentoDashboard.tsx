...
export default function BudgetBentoDashboard({ draft, locale }: BudgetBentoDashboardProps) {
  ...
  const dailyAverage = Math.round(totalBudgetKrw / (nights || 1));
  ...
  const categories = [
    {
      ...
      labelEn: "Stay",
      pct: 42,
      amount: Math.round(totalBudgetKrw * 0.42),
    ...
    },
    {
      ...
      labelEn: "Dining",
      pct: 28,
      amount: Math.round(totalBudgetKrw * 0.28),
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 max-w-5xl mx-auto mt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      
      {/* ========================================================================= */}
      {/* 1. Card A: 총 예산 및 카테고리 링 차트 (col-span-1 md:col-span-2 p-6 sm:p-8) */}
      {/* ========================================================================= */}
      <div className="col-span-1 md:col-span-2 p-6 sm:p-8 bg-white/80 backdrop-blur-md rounded-3xl border border-neutral-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_16px_40px_rgb(0,0,0,0.06)] transition-all duration-300 flex flex-col justify-between">
        <div>
          {/* Top Label */}
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-neutral-400">
              {locale === "ko" ? "TOTAL ESTIMATED BUDGET (KRW)" : "TOTAL ESTIMATED BUDGET (KRW)"}
            </span>
          ...
          <div className="mt-2 flex flex-wrap items-baseline gap-2 sm:gap-3">
            ...
            <span className="text-xs font-semibold text-neutral-500 bg-neutral-100/80 px-2.5 py-0.5 rounded-full border border-neutral-200/60">
              {locale === "ko"
                ? `1인당 ${perPersonTotal.toLocaleString()}원`
                : `${perPersonTotal.toLocaleString()} KRW/person`}
            </span>
          </div>
        </div>

        {/* Center Content: Minimal SVG Donut / Ring Chart & Legend */}
        <div className="mt-6 pt-6 border-t border-neutral-100 grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
          ...
          <div className="sm:col-span-7 grid grid-cols-2 gap-2.5">
            {categories.map((cat) => (
              <div
                key={cat.key}
                className="flex flex-col p-2.5 rounded-2xl bg-neutral-50/60 border border-neutral-200/50 hover:bg-neutral-100/60 transition-colors"
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[11px] font-bold text-neutral-400 ml-auto">
                    {cat.pct}%
                  </span>
                </div>
      ...
      {/* ========================================================================= */}
      <div className="col-span-1 p-6 flex flex-col justify-between bg-white/80 backdrop-blur-md rounded-3xl border border-neutral-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden group hover:-translate-y-1 hover:shadow-[0_16px_40px_rgb(0,0,0,0.06)] transition-all duration-300">
        <div>
          {/* Top Label */}
        ...
        <div className="pt-3 mt-3 border-t border-neutral-100 flex items-center justify-between">
          <Link
            ...
            className="text-xs font-bold text-teal-600 hover:text-teal-700 inline-flex items-center gap-1 group-hover:underline cursor-pointer"
          >
      ...
      {/* ========================================================================= */}
      <div className="col-span-1 p-6 bg-white/80 backdrop-blur-md rounded-3xl border border-neutral-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_16px_40px_rgb(0,0,0,0.06)] transition-all duration-300 flex flex-col justify-between">
        <div>
          {/* Top Label */}
          ...
          {/* Metric: 1일 평균 예상 비용 */}
          <div className="mt-3">
            <span className="text-2xl sm:text-3xl font-bold tracking-tight text-neutral-900 block">
              ~{dailyAverage.toLocaleString()} KRW
            </span>
        ...
        <p className="text-[11px] text-neutral-400 mt-4 leading-relaxed">
          {locale === "ko"
            ? `1인당 1일 약 ${Math.round(dailyAverage / adults).toLocaleString()}원으로 여유롭게 즐길 수 있는 추천 견적입니다.`
            : `Comfortable daily pacing of approx. ${Math.round(dailyAverage / adults).toLocaleString()} KRW per traveler.`}
        </p>
      </div>
      ...
      {/* ========================================================================= */}
      <div className="col-span-1 md:col-span-2 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-5 bg-neutral-50/80 rounded-3xl border border-neutral-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_16px_40px_rgb(0,0,0,0.06)] transition-all duration-300">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            type="button"
            onClick={handleShare}
            className="bg-neutral-900 text-white hover:bg-neutral-800 active:scale-95 px-5 py-2.5 rounded-full text-sm font-medium transition-all shadow-sm flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
          >
            {copied ? (
              <>
                ...
                <span>{locale === "ko" ? "링크 복사됨!" : "Link Copied!"}</span>
              </>
            ) : (
              <>
                ...
                <span>{locale === "ko" ? "일정 링크 복사" : "Share Plan"}</span>
              </>
            )}
          </button>
        </div>
